...
public class MovieViewModel extends ViewModel {
    ...
    private MutableLiveData<ArrayList<Movie>> mListMovie = new MutableLiveData<>();
    ...
    public void setMovie() {
        ...
        String mUrl = BASE_URL + MOVIE_URL + API_KEY + "&language=en-US";
        mClient.get(mUrl, new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                try {
                    String mResult = new String(responseBody);
                    JSONObject mResponseObject = new JSONObject(mResult);
                    JSONArray listMovie = mResponseObject.getJSONArray("results");
                    for (int i = 0; i < listMovie.length(); i++) {
                        JSONObject mMovies = listMovie.getJSONObject(i);
                    ...
                    mListMovie.postValue(mListItemsMovie);
                } catch (Exception e) {
                    Log.d("Exception", e.getMessage());
                }
            }
